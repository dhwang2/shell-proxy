...
_proxy_bundle_build_script() {
    # Emits the build-time script executed in the bundle subshell. Kept as a
    # separate function so both timeout-wrapped and direct invocations share
    # one source of truth.
    cat <<'EOF'
set -Eeuo pipefail
IFS=$'\n\t'
...
proxy_rebuild_menu_bundle_impl() {
    ...
    if [[ -n "$timeout_bin" ]]; then
        argv=("$timeout_bin" "$timeout_seconds" "$bundle_shell")
    else
        argv=("$bundle_shell")
    fi

    if ! PROXY_BUNDLE_ROOT_DIR="$root_dir" PROXY_BUNDLE_GROUP_NAME="$group" \
        "${argv[@]}" >"$tmp_path" < <(_proxy_bundle_build_script); then
